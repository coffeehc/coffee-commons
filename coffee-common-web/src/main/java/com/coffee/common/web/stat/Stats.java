...
/**
 * This class represents the collection of registered stats within an injector.
 * Its main roles are to act as a container for these stats, and to provide
 * access to them through its {@link #snapshot()} method.
 *
...
final class Stats implements StatsSnapshotter {
	...
	private final ConcurrentMap<String, StatDescriptor>	stats					= new MapMaker()
																						.makeMap();
	...
	void register(StatDescriptor statDescriptor) {
		...
		StatDescriptor existingDescriptor = stats.putIfAbsent(statName,
				statDescriptor);

		if (existingDescriptor != null
				&& !(existingDescriptor.getStatReader().equals(statDescriptor
						.getStatReader()))) {
			...
			StatDescriptor syntheticDescriptor = StatDescriptor.of(statName,
					"Placeholder for duplicate stat",
	...
	public ImmutableMap<StatDescriptor, Object> snapshot() {
		...
		ImmutableMap.Builder<StatDescriptor, Object> builder = ImmutableMap
				.builder();
		for (StatDescriptor statDescriptor : stats.values()) {
			...
			StatExposer statExposer = getStatExposer(statDescriptor
					.getStatExposerClass());
			@SuppressWarnings("unchecked")
			// We know we don't guarantee a <T> here.
	...
	private void checkBindingsExistForExposers() {
		...
		Set<Class<? extends StatExposer>> statExposerClasses = Sets
				.newHashSet();
		for (StatDescriptor statDescriptor : stats.values()) {
