 ...
 */
class StatCollector implements
		Function<Class<?>, List<MemberAnnotatedWithAtStat>> {
	 ...
	 *
	 * <p>
	 * Climbs the class hierarchy finding all annotated members.
	 */
	@Override
	public List<MemberAnnotatedWithAtStat> apply(Class<?> clazz) {
		...
		for (Class<?> currentClass = clazz; currentClass != Object.class; currentClass = currentClass
			...
			for (Method method : currentClass.getDeclaredMethods()) {
				...
				if (stat != null && staticMemberPolicy.shouldAccept(method)) {
					annotatedMembers.add(new MemberAnnotatedWithAtStat(stat,
							method));
				}
			}
