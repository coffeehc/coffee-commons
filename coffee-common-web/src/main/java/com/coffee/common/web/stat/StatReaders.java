...
public final class StatReaders {
	...
	public static StatReader forMethod(final Method method, final Object target) {
	...
	public static StatReader forMember(Member member, Object target) {
		...
		throw new IllegalArgumentException("Unsupported type of member: "
				+ member);
	}
	...
	private static class FieldBasedStatReader extends StatReader {
		...
		@Override
		boolean equalsOtherStatReader(StatReader otherStatReader) {
			if (!FieldBasedStatReader.class.isInstance(otherStatReader)) {
				return false;
			}
			FieldBasedStatReader that = (FieldBasedStatReader) otherStatReader;
		...
		public String toString() {
			return Objects.toStringHelper(this).add("field", field)
	...
	private static class MethodBasedStatReader extends StatReader {
		...
		boolean equalsOtherStatReader(StatReader otherStatReader) {
			...
			MethodBasedStatReader that = (MethodBasedStatReader) otherStatReader;
	...
	private static class ObjectBasedStatReader extends StatReader {
		...
		boolean equalsOtherStatReader(StatReader otherStatReader) {
			...
			ObjectBasedStatReader that = (ObjectBasedStatReader) otherStatReader;
		...
		@Override
		public String toString() {
			return Objects.toStringHelper(this).add("object", object)
					.toString();
		}
	}
}
